...
const registerIOSModule = require("./ios/registerNativeModule");
// import type { RNConfig } from "../core";

log.heading = "linknopod";
...
const linkDependency = async (platforms, project, dependency) => {
  ...
  Object.keys(platforms || {}).forEach(platform => {
    ...
    if (platform == "ios") {
      //Change of plans
      console.log(
        "I will register IOS directly",
        dependency.name,
        dependency.config[platform],
        project[platform]
      );
      registerIOSModule(dependency.config[platform], project[platform]);
    } else {
      console.log("O fuck.", platform);
      process.exit();
      linkConfig.register(
        dependency.name,
        dependency.config[platform],
...
module.exports = {
  func: link,
  description:
    "links as static libraries all native dependencies (updates native build files)",
  name: "linknopod [packageName]"
};
